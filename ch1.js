...
//The init funcion is defined down the code 
...
document.querySelector('.btn-roll').addEventListener('click', function() {
    ...
    var diceDomb = document.querySelector('#dice1');
    diceDom.style.display= 'block';
    diceDom.src= 'dice-'+dice+'.png';
        
    diceDomb.style.display= 'block';
    diceDomb.src= 'dice-'+dice1+'.png';
    
   
    //Update the round the score only if the score is not 1 otherwise its the next players turn
     //if the the round score is greater than 10 and if one of the die is an even number
     // the global score for that player is subtracted by double of the sum of the two die values 
     // then it is immediatly the next players turn 
     if (dice !== 1 && dice1!==1) {
         if ((roundScore > 10)&&((dice === 2 ) ||(dice === 4) ||(dice === 6) || (dice === 2 ) ||(dice === 4) ||(dice === 6) )) {
             scores[activePlayer]-= 2* (dice + dice1);
             document.querySelector('#current-'+activePlayer).textContent=(2*(dice+dice1));
             ...
             //The roundscore is the sum of the two die
...
function nextplayer () {
    ...
    //Set prevscore back to 0 when next turn starts 
